...
router.get('/my/carts', async (ctx)=>{
  ...
  let res = await db.query(
    `SELECT a.id,a.goods_sku_id,a.goods_id,a.num,a.goods_sku_desc,b.goods_attr_path,b.price,b.stock,c.goods_name,c.goods_desc 
...
router.put('/my/carts/:id', async (ctx)=>{
  ...
  if (hasExistRes){
    console.log('hasExistRes',hasExistRes);
    // 返回成功更新的个数，是一个数组
    // 返回[0]代表失败
    res = await GoodsCarts.update({
      num:num
...
router.delete('/my/carts', async (ctx)=>{
  ...
  console.log('ids',ids, ctx.request.body);
  let res = GoodsCarts.destroy({
    where:{
      id:ids
    }
  })
